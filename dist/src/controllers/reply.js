...
const createReply = async (req, res) => {
    ...
    if (!threadId || Number.isNaN(threadId))
        throw new appError_1.AppError(400, "Invalid thread id");
    const content = String(req.body.content ?? "");
    if (!content.trim())
        throw new appError_1.AppError(400, "Content is required");
    const file = req.file;
    const imageUrl = file
        ? `${req.protocol}://${req.get("host")}/uploads/${file.filename}`
        : null;
    const reply = await client_1.prisma.reply.create({
        data: {
            user_id: userId,
            thread_id: threadId,
            content,
    ...
    await client_1.prisma.thread.update({
        where: { id: threadId },
